...
def process_ppg_file(file_path):
    ...
    if os.path.basename(file_path) == "26_5pinky.csv":
        fs = 400  # Hz
        f, tt, Sxx = spectrogram(y_fixed, fs=fs, nperseg=8192, noverlap=4096)

        # Time vector with correct length
        t = np.arange(len(y_fixed)) / fs
        ...
        # Time domain PPG
        ax[0].plot(t, y_fixed, color='darkred')
        ax[0].set_title("Filtered PPG Signal (Time Domain)")
        ax[0].set_ylabel("Amplitude")

        # Frequency domain spectrogram (PSD vs time)
        im = ax[1].pcolormesh(tt, f, Sxx_dB, shading='gouraud', cmap='magma')
        ax[1].set_title("Spectrogram (Power Spectral Density)")
        ax[1].set_xlabel("Time [s]")
        ax[1].set_ylabel("Frequency [Hz]")
        fig.colorbar(im, ax=ax[1], label='PSD [dB/Hz]')
